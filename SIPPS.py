...
def insert_node(node, open_list, closed_list, h_values, soft_obstacle, count_tie_break):
    ...
    if combined_heuristic in count_tie_break:
        ...
        heapq.heappush(
            open_list, ((combined_heuristic[0], combined_heuristic[1], tie_break), node))
    else:
        count_tie_break[combined_heuristic] = 1
        heapq.heappush(
            open_list, ((combined_heuristic[0], combined_heuristic[1], 0), node))
...
def expand_node(my_map, curr_node, open_list, closed_list, safe_interval_table, hard_obstacle, soft_obstacle, h_values, count_tie_break):
    ...
    valid_neighbors = get_valid_nodes(
        my_map, curr_loc, node_low, node_high, safe_interval_table)
    ...
    for (next_loc, interval_id) in valid_neighbors:
        ...
        if (curr_loc, next_loc) in hard_obstacle:
            ...
            # uncolide with hard_obstacle([curr_loc, next_loc])
            low = get_earlieset_arrival_time(
                (curr_loc, next_loc), low, high, hard_obstacle, None)

            if low is None:
                continue

        earliest_low = get_earlieset_arrival_time(
            (curr_loc, next_loc), low, high, hard_obstacle, soft_obstacle)  # uncolide with hard, soft obstacle
        ...
        if earliest_low is not None and earliest_low > low:
            ...
            insert_node(n1, open_list, closed_list, h_values,
                        soft_obstacle, count_tie_break)

            n2 = {'c_val': 0, 'loc': next_loc, 'g_val': 0,
                  'h_val': h_values[next_loc], 'interval': (earliest_low, high),
                  'id': interval_id, 'is_goal': False, 'parent': curr_node}
            insert_node(n2, open_list, closed_list, h_values,
                        soft_obstacle, count_tie_break)
        else:
            n1 = {'c_val': 0, 'loc': next_loc, 'g_val': 0,
...
def is_contain_obstacle(node_loc, interval, soft_obstacle):
    ...
    if node_loc in soft_obstacle:
        temp_times = list(soft_obstacle[node_loc].copy())
...
def is_contain_edge(parent_loc, node_loc, n_low, soft_obstacle):
    ...
    if (parent_loc, node_loc) in soft_obstacle:
        ...
        temp_times = list(soft_obstacle[(parent_loc, node_loc)].copy())
...
def get_c_val(node, closed_list, soft_obstacle):
    ...
    node_loc = node['loc']
    cv = 1 if is_contain_obstacle(
        node_loc, node_interval, soft_obstacle) else 0

    n_low = node['interval'][0]
    # n_edge = (parent_node['loc'], node['loc'])
    ce = 1 if is_contain_edge(
        parent_node['loc'], node_loc, node_interval[0], soft_obstacle) else 0
...
def get_c_future(curr_loc, soft_obstacle, n_low):
    ...
    if curr_loc in soft_obstacle:
        temp_times = list(soft_obstacle[curr_loc].copy())

        while len(temp_times) > 0:
            time = heapq.heappop(temp_times)
            if time > n_low:
                # Since all of rest of times in temp_times will be greater than n_low from now,
                c_future = len(temp_times) + 1
                break
    return c_future
...
def sipps(my_map, start_loc, goal_loc, h_values, hard_obstacle, soft_obstacle):
    ...
    safe_interval_table = build_safe_interval_table(
        my_map, soft_obstacle, hard_obstacle)  # my_map is avaialble paths excluding walls
    # print(safe_interval_table)

    temp1 = h_values[start_loc]
    temp2 = safe_interval_table[start_loc][0]

    root = {'c_val': 0, 'loc': start_loc, 'g_val': 0,
            'h_val': h_values[start_loc], 'interval': safe_interval_table[start_loc][0], 'id': 1, 'is_goal': False, 'parent': None}
    ...
    while len(open_list) > 0:
        ...
        if curr['loc'] == goal_loc and curr['interval'][0] >= lower_bound_timestep:
            c_future = get_c_future(
                curr['loc'], soft_obstacle, curr['interval'][0])
            ...
            insert_node(updated_node, open_list,
                        closed_list, h_values, soft_obstacle, count_tie_break)

        expand_node(my_map, curr, open_list, closed_list, safe_interval_table,
                    hard_obstacle, soft_obstacle, h_values, count_tie_break)
        ...
        if combined_heuristic in count_tie_break:
            ...
            count_tie_break[combined_heuristic] += 1
            heapq.heappush(
                closed_list, ((combined_heuristic[0], combined_heuristic[1], tie_break), curr))
        else:
            count_tie_break[(curr['c_val'], curr['g_val'])] = 1
            heapq.heappush(
                closed_list, ((combined_heuristic[0], combined_heuristic[1], 0), curr))
